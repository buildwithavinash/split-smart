...
const GroupInputForm = ({
  ...
  const handleClick = () => {
    ...
    if (members.length < 2) {
      setErrorMsg("Add at least 2 members to create a group.");
      return;
    }
    setErrorMsg("");
    createGroup();
  ...
  return (
    <div className="p-2 absolute inset-0 h-screen w-full backdrop-blur-xl flex flex-col justify-center items-center z-10">
      ...
      >
          ...
          {members.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              {members.map((m, id) => (
                <div
                  key={id}
                  className="bg-slate-300 text-slate-900 flex items-center px-3 py-1 rounded-xl justify-center gap-2"
                >
                  <span>{m}</span>
                  <button
                    onClick={(e) => {
        ...
        {errorMsg && <p className="text-sm text-red-500 my-1">{errorMsg}</p>}
        <button
          type="submit"
          onClick={handleClick}
        ...
        {isFormOpen && (
          <button
            ...
            className="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded-lg font-semibold transition-all cursor-pointer border border-slate-500 -mt-1"
          >
            Cancel
          </button>
        )}
